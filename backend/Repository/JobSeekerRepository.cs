...
namespace backend.Repository
{
    public class JobSeekerRepository
    {
        ...
        public List<JobVacancy> GetRecommendedJobs(int seekerId)
        {
            connection();
            List<JobVacancy> jobs = new List<JobVacancy>();

            // Step 1: Get the seeker's skills
            List<int> seekerSkills = new List<int>();
            SqlCommand getSkillsCmd = new SqlCommand("SELECT SkillID FROM JobSeekerSkills WHERE SeekerID = @SeekerID", con);
            getSkillsCmd.Parameters.AddWithValue("@SeekerID", seekerId);

            con.Open();
            SqlDataReader reader = getSkillsCmd.ExecuteReader();
            while (reader.Read())
            {
                seekerSkills.Add(Convert.ToInt32(reader["SkillID"]));
            }
            reader.Close();
            ...
            {
                con.Close();
                return jobs; // No skills = no recommendations
            }

            // Step 2: Get recommended jobs that match the seeker's skills and not applied to yet
            string skillIdList = string.Join(",", seekerSkills);

            string query = @"
        SELECT DISTINCT JV.*
        FROM JobVacancies JV
        JOIN JobSkills JS ON JV.JobID = JS.JobID
        WHERE JS.SkillID IN (" + skillIdList + @")
         AND JV.IsPublished = 1
         AND JV.JobID NOT IN (
             SELECT JobID FROM JobApplications WHERE SeekerID = @SeekerID
          )
        ORDER BY JV.IsPublished DESC";
            ...
            while (jobReader.Read())
            {
                jobs.Add(new JobVacancy
                {
                    ...
                    JobTitle = jobReader["JobTitle"].ToString(),
                    Description = jobReader["Description"].ToString(),
            ...
            con.Close();
            return jobs.Take(3).ToList(); // limit to 10 recommendations
        }
